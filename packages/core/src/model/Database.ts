...
export class ModelDatabase {
  ...
  private prepareStatements(): void {
    ...
    this.statements.set('provider_get', this.db.prepare(
      "SELECT * FROM providers WHERE id = ? AND tool_id = COALESCE(?, 'global')"
    ));
    ...
    this.statements.set('provider_get_by_id', this.db.prepare(
      'SELECT * FROM providers WHERE id = ? LIMIT 1'
    ));

    this.statements.set('provider_get_global', this.db.prepare(
      "SELECT * FROM providers WHERE is_global = 1 OR tool_id = 'global' ORDER BY priority DESC, name ASC"
    ));

    this.statements.set('provider_get_by_tool', this.db.prepare(
      'SELECT * FROM providers WHERE tool_id = ? ORDER BY priority DESC, name ASC'
    ));

    this.statements.set('provider_get_current_global', this.db.prepare(
      'SELECT * FROM providers WHERE is_global = 1 AND is_current_global = 1 LIMIT 1'
    ));

    this.statements.set('provider_get_current_tool', this.db.prepare(
      'SELECT * FROM providers WHERE tool_id = ? AND is_current_tool = 1 LIMIT 1'
    ));

    this.statements.set('provider_delete', this.db.prepare(
      "DELETE FROM providers WHERE id = ? AND tool_id = COALESCE(?, 'global')"
    ));
    ...
    this.statements.set('api_key_get_global', this.db.prepare(
      "SELECT * FROM api_keys WHERE provider_id = ? AND provider_tool_id = 'global'"
    ));

    this.statements.set('api_key_get_by_tool', this.db.prepare(
      'SELECT * FROM api_keys WHERE provider_id = ? AND provider_tool_id = ?'
    ));
